        ...
        async function loadProfileData() {
            ...
            try {
                ...
                if (data && !data.error) {
                    ...
                    const avatarImg = document.querySelector('.profile-card-avatar-img');
                    if (avatarImg && data.profile_pic_url) {
                        avatarImg.src = data.profile_pic_url;
                    }
                    ...
                    if (nameEl) {
                        nameEl.textContent = '@' + cleanUsername;
                    }
                    
                    // Atualizar estatísticas
                    const stats = document.querySelectorAll('.profile-card-stat-number');
                    if (stats.length >= 3) {
                        stats[0].textContent = data.media_count || 0;  // Posts
                        stats[1].textContent = data.follower_count || 0;  // Seguidores
                        stats[2].textContent = data.following_count || 0;  // Seguindo
                    }
                }
            } catch (error) {
